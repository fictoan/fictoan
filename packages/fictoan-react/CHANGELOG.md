...
- Extend `inheritFormSpacing` so a generic wrapper (`<Div inheritFormSpacing>`, `<Card inheritFormSpacing>`, etc.) also acts as a vertical flow container for its own children, using `--form-spacing` as the gap. Native form components (`FormItem`, `FormItemGroup`, `Checkbox`, `Switch`) are excluded from this rule so their existing layouts stay intact. Lets FormItems inside wrapper cells (e.g. inside a `FormItemGroup columns={2}`) space themselves at the Form's rhythm without any ancestor reset working against them
- Scale `InputField` inner icons with the `size` prop: add the missing `size-large` variant (`--input-icon-size: 32px`), and force the icon wrapper's direct child and any inner `<svg>` to `width/height: 100%` so library icons (lucide-react, etc.) with hard-coded intrinsic dimensions follow the input's size instead of sitting at a fixed 24px
